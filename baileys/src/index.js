...
function parseMsgOLD(msg) {
  ...
  if (c?.conversation || c?.extendedTextMessage) {
    type = 'text';
    data = { text: c.conversation || c.extendedTextMessage?.text };
  } else if (c?.imageMessage)    { type = 'image';    data = { caption: c.imageMessage.caption }; }
  else if (c?.videoMessage)      { type = 'video';    data = { caption: c.videoMessage.caption }; }
  else if (c?.audioMessage)      { type = 'audio';    data = {}; }
  else if (c?.documentMessage)   { type = 'document'; data = { fileName: c.documentMessage.fileName }; }
  else if (c?.buttonsMessage) {
    // הודעת כפתורים נכנסת
    type = 'buttons';
    data = {
      text: c.buttonsMessage.text || c.buttonsMessage.contentText,
      footer: c.buttonsMessage.footerText,
      buttons: c.buttonsMessage.buttons?.map(b => ({
        id: b.buttonId,
        text: b.buttonText?.displayText
      }))
    };
  } else if (c?.buttonResponseMessage) {
    type = 'button_response';
    data = { buttonId: c.buttonResponseMessage.selectedButtonId,
             displayText: c.buttonResponseMessage.selectedDisplayText };
  } else if (c?.listResponseMessage) {
    type = 'list_response';
    data = { rowId: c.listResponseMessage.singleSelectReply?.selectedRowId,
             title: c.listResponseMessage.title };
  } else if (c?.templateButtonReplyMessage) {
    type = 'template_button_response';
    data = { selectedId: c.templateButtonReplyMessage.selectedId,
             displayText: c.templateButtonReplyMessage.selectedDisplayText };
  } else if (c?.reactionMessage) {
    type = 'reaction';
    data = { emoji: c.reactionMessage.text };
  } else if (c?.locationMessage) {
    type = 'location';
  ...
  return {
    messageId: msg.key.id, jid, sender, isGroup,
    timestamp: msg.messageTimestamp, type, data,
    receivedAt: new Date().toISOString()
  };
}
