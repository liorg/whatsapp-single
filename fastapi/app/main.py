...
- **Templates**: createTemplate, FindStatusTemplate
""",
    lifespan=lifespan
)
...
# ── Templates ─────────────────────────────────────────────────────────────────
@app.post("/templates", tags=["Templates"], operation_id="createTemplate")
async def create_template(b: TemplateCreate):
    tid = f"{int(time.time() * 1000)}{secrets.randbelow(1000):03d}"
    rec = {**b.model_dump(exclude_none=True), "id": tid, "status": "PENDING"}
    await redis_client.set(f"wa:templates:{PHONE_ID}:{tid}", json.dumps(rec, ensure_ascii=False))
    return {"id": tid, "status": "PENDING", "category": b.category}

@app.get("/templates/{template_id}", tags=["Templates"], operation_id="FindStatusTemplate")
async def find_status_template(template_id: str):
    ...
    if rec["status"] == "PENDING":
        rec["status"] = "APPROVED"
        await redis_client.set(key, json.dumps(rec, ensure_ascii=False))
